...
const derivedDatabaseURL = requiredFirebaseEnv.projectId
  ? `https://${requiredFirebaseEnv.projectId}-default-rtdb.firebaseio.com`
  : '';
const derivedStorageBucket = requiredFirebaseEnv.projectId
  ? `${requiredFirebaseEnv.projectId}.appspot.com`
  : '';
...
export const validateClientEnv = () => {
  ...
  if (missing.length) {
    throw new Error(
      `Invalid Firebase environment variables: ${missing.join(', ')}. Use KEY=value with no quotes, spaces, or commas.`
    );
  }
};
